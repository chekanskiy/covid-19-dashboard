...
def build_hierarchical_dataframe(df, levels, value_column, color_columns=None):
    ...
    for i, level in enumerate(levels):
        ...
        if (
            i < len(levels) - 1
        ):  # Setting PARENT value, the next column untill the last one, after which it is TOTAL
    ...
    if color_columns is not None:
        ...
        total_dict = dict(id="total", parent="", value=df[value_column].sum(),)
...
def plot_sunburst_static(
    ...
    fig.update_layout(
        margin=dict(t=10, b=10, r=10, l=10), plot_bgcolor=color, paper_bgcolor=color,
    )
    return fig
