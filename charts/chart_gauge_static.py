...
def plot_gauges(df, value_column, aggregate_by_column="region_wb"):
    ...
    if "_per_100k" in value_column:
        ...
        reg_df_sum_population = df.groupby(aggregate_by_column).agg(["sum"])[
            "population_100k"
        ]
        ...
        reg_df_max = (
            ...
            .groupby(["date", aggregate_by_column])
            .agg(["sum"])[column_original]
            .groupby(aggregate_by_column)
            .agg(["max"])
        )
        ...
        yd_reg_df_sum = (
            df.loc[df.date == df.date.max() - timedelta(days=1), :]
            .groupby(aggregate_by_column)
            .agg(["sum"])[column_original]
        )
        ...
        current_reg_df_sum["sum"] = (
            current_reg_df_sum["sum"] / current_reg_df_sum["population"]
        )
    ...
    for i, region in enumerate(aggregated_categories[:display_values]):
        ...
        max_value = (
            reg_df_max.reset_index()
            .loc[reg_df_max.index == region, "max"]
            .round(3)
            .values[0]
        )
        current_value = (
            current_reg_df_sum.reset_index()
            .loc[current_reg_df_sum.index == region, "sum"]
            .round(3)
            .values[0]
        )
        yd_value = (
            yd_reg_df_sum.reset_index()
            .loc[yd_reg_df_sum.index == region, "sum"]
            .round(3)
            .values[0]
        )
    ...
    fig.update_layout(
        ...
        margin=dict(autoexpand=True, l=0, r=0, t=50, b=10,),
    )
